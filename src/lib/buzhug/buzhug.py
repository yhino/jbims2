    ...
    def __repr__(self):
        ...
        for k in self.fields:
            if not isinstance(getattr(self,k),unicode):
                ...
                elts.append(('%s:%s' %(k,getattr(self,k))).encode('latin-1'))
        return '<' + ' '.join(elts) +'>'
...
class Base:
    ...
    def update(self,record,**kw):
        ...
        if only_fixed_length:
            ...
            new_pos = {}
            # copy blocks at the end of field files for fields not updated
            unset_fields = [ (i,f) for (i,f) in enumerate(self.field_names)
                if not hasattr(record,f) ]
            for i,f in unset_fields:
                pos = field_pos[i]
